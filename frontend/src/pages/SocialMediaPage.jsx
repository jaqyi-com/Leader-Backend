    ...
    >
      {tags.map((t, i) => (
        <span
          ...
          style={{ background: "rgba(226,55,68,0.15)", color: "var(--accent-2)" }}
        >
          <Hash size={10} /> {t}
          {!disabled && (
        ...
        return (
          <div key={s} className="flex items-center gap-1">
            <div
              ...
              style={{
                background: done ? "rgba(16,185,129,0.15)" : current ? "rgba(226,55,68,0.2)" : "var(--surface-2)",
                color:      done ? "var(--emerald)"        : current ? "var(--accent-2)"       : "var(--text-3)",
                border:     `1px solid ${done ? "rgba(16,185,129,0.3)" : current ? "rgba(226,55,68,0.35)" : "var(--border)"}`,
              }}
            >
              ...
              {post.hashtags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {post.hashtags.map((h, i) => (
                    <span key={i} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(226,55,68,0.1)", color: "var(--accent-2)" }}>
                      #{h.replace(/^#/, "")}
                    </span>
                  ))}
                </div>
    ...
    <div className="flex flex-col gap-6 h-full max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className="w-10 h-10 rounded-xl flex items-center justify-center"
            style={{ background: "linear-gradient(135deg, #E23744 0%, #f47a88 100%)", boxShadow: "0 0 20px rgba(226,55,68,0.35)" }}
          >
        ...
        <div className="xl:col-span-1 flex flex-col gap-4">
          <div className="card p-5">
            ...
            <div
              ...
              style={{ background: "rgba(226,55,68,0.05)", border: "1px solid rgba(226,55,68,0.15)", color: "var(--text-3)" }}
            >
                  ...
                  <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {keywords.map((k, i) => (
                            <span key={i} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(226,55,68,0.15)", color: "var(--accent-2)" }}>#{k}</span>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-2.5 text-xs" style={{ color: "var(--text-2)" }}>
                        <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(226,55,68,0.15)", color: "var(--accent-2)" }}>
                          {i + 1}
                        </div>
                        {step}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {generated.hashtags.map((h, i) => (
                          <span key={i} className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: "rgba(226,55,68,0.12)", color: "var(--accent-2)" }}>
                            #{h.replace(/^#/, "")}
                          </span>
                        ))}
                      </div>
          ...
          <div className="card flex-1 flex flex-col min-h-0">
            <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: "var(--border)" }}>
              <h2 className="font-semibold text-sm flex items-center gap-2" style={{ color: "var(--text)" }}>
                ...
                Post History
                {posts.length > 0 && (
                  <span className="ml-1 text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(226,55,68,0.15)", color: "var(--accent-2)" }}>
                    {posts.length}
                  </span>
                )}
              </h2>

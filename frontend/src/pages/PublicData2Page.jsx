...
import {
  ...
  Globe, Users, Building2, Briefcase, Tag,
} from "lucide-react";
import { n2GetDatabase, n2GetStats, n2Refresh } from "../api";
...
// Priority columns — matching real niches_75m_data field names
const PRIORITY_COLS = [
...
export default function PublicData2Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await n2GetDatabase({
        page, limit,
      ...
      toast.error("Failed to load Niches 75M data");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    n2GetStats().then(({ data }) => setStats(data)).catch(() => {});
  }, []);
  ...
  const handleRefresh = async () => {
    ...
    try {
      await n2Refresh();
      await Promise.all([load(), n2GetStats().then(({ data }) => setStats(data))]);
  ...
  const statRows = stats ? [
    ...
    { label: "Niches",         val: stats.niche_count,  icon: Tag,       color: "#a855f7" },
  ] : [];
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[var(--text)] flex items-center gap-2">
            ...
            Public Data 2 — 75M Niches
          </h2>
          <p className="text-sm text-[var(--text-3)] mt-0.5">
            {total.toLocaleString()} records · Live Cloud SQL ·{" "}
            <span className="font-mono text-[11px] opacity-60">niches_75m_data</span>
          </p>
        </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="card p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              <div className="relative col-span-2">
                ...
                <input className="input pl-8 text-xs w-full" placeholder="Search name, company, email, niche…" value={filters.search} onChange={e => setF("search", e.target.value)} />
              </div>
              ...
              <input className="input text-xs" placeholder="Niche (e.g. roofing)…" value={filters.niche} onChange={e => setF("niche", e.target.value)} />
                  ...
                  <tr key={`skel-${idx}`} className="border-b border-[var(--border)] animate-pulse">
                    {PRIORITY_COLS.map((_, ci) => (
                      <td key={ci} className="px-3 py-3">
                        <div className="h-4 bg-[var(--surface-2)] rounded" style={{ width: `${55 + (ci * 17) % 65}px` }} />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                <tr key={i} className="border-b border-[var(--border)] hover:bg-[var(--surface-2)] transition-colors">
                  {PRIORITY_COLS.map(c => {
                    ...
                    if (c.key === "email") {
                      ...
                      return (
                        <td key={c.key} className="px-3 py-2.5">
                          <MultiValueCell values={emails} hrefFn={em => `mailto:${em}`} Icon={Mail} color="#60a5fa" />
                        </td>
                      );
                    }
                    ...
                    if (c.key === "phone") {
                      ...
                      return (
                        <td key={c.key} className="px-3 py-2.5">
                          <MultiValueCell values={phones} hrefFn={ph => `tel:${ph}`} Icon={Phone} color="#34d399" />
                        </td>
                      );
                    }
                    ...
                    if (c.key === "linkedin") return (
                      <td key={c.key} className="px-3 py-2.5">
                        {val
                          ? <a href={val.startsWith("http") ? val : `https://${val}`} target="_blank" rel="noreferrer"
                               className="text-blue-400 hover:text-blue-300 text-[10px] underline truncate max-w-[100px] block" title={val}>
                              LinkedIn
                            </a>
                          : "—"}
                      </td>

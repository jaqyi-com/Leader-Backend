...
import {
  Phone, Search, Download, RefreshCw,
  Mail, MapPin, Globe, Building2, Database, Users2,
} from "lucide-react";
import {
  fpnGetDatabase, fpnGetStats, fpnGetColumns, fpnRefresh,
...
function renderCell(col, val) {
  ...
  if (key.includes("phone") || key === "mobile") return (
    <a href={`tel:${val}`}
      className="flex items-center gap-1 text-[11px] text-green-400 hover:underline max-w-[200px] truncate"
      title={val}>
...
function TablePanel({ mode }) {
  ...
  const getDB     = isPeople ? fpnGetDatabase : (p) => fcGetDatabase({ ...p, f_has_phone: "true" });
  const getCols   = isPeople ? fpnGetColumns  : fcGetColumns;
  const doRefresh = isPeople ? fpnRefresh     : fcRefresh;
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-3)] mt-0.5">
            Browse {mode === "people" ? "people" : "companies"} who have phone numbers on record
          </p>
        </div>
      </div>

...
import { indiaGetDatabase, indiaGetStats, indiaGetColumns, indiaRefresh } from "../api";
...
export default function IndiaDataPage() {
  ...
  const loadColumns = useCallback(async () => {
    try {
      const { data } = await indiaGetColumns();
      ...
      // fallback — will just show empty table header
    }
  }, []); // eslint-disable-line
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await indiaGetDatabase({
        page, limit,
      ...
      toast.error("Failed to load India Data");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    indiaGetStats().then(({ data }) => setStats(data)).catch(() => {});
  }, []);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      await Promise.all([
        loadColumns(),
        load(),
        indiaGetStats().then(({ data }) => setStats(data)),
      ]);
  ...
  // Cell renderer — smart formatting based on common column patterns
  const renderCell = (col, val) => {
    ...
    if (key.includes("website") || key === "url" || key === "web") return (
    ...
    if (key.includes("name") || key === "first_name" || key === "last_name") return (
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[var(--text)] flex items-center gap-2">
            <Flag size={22} className="text-[#FF9933]" />
            India Data
          </h2>
          <p className="text-sm text-[var(--text-3)] mt-0.5">
            {total.toLocaleString()} records · Live Cloud SQL ·{" "}
            <span className="font-mono text-[11px] opacity-60">india_data</span>
          </p>
        </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="card p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              <div className="relative col-span-2">
                ...
                <input className="input pl-8 text-xs w-full" placeholder="Search across all columns…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
              </div>

...
import {
  ...
  Mail, MapPin, ChevronDown, ChevronUp, X,
  Globe, Building2, Database, Users,
} from "lucide-react";
import { fpnGetDatabase, fpnGetStats, fpnGetColumns, fpnRefresh } from "../api";
import toast from "react-hot-toast";

export default function PeopleNumberPage() {
  ...
  const loadColumns = useCallback(async () => {
    try {
      const { data } = await fpnGetColumns();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await fpnGetDatabase({
        page, limit,
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    fpnGetStats().then(({ data }) => setStats(data)).catch(() => {});
  }, []);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      await Promise.all([
        loadColumns(),
        load(),
        fpnGetStats().then(({ data }) => setStats(data)),
      ]);
  ...
  const exportCSV = () => {
    ...
    a.download = "people_number_data.csv";
    a.click();
  };

  const pages    = Math.ceil(total / limit);
  const statRows = stats ? [
    { label: "People with Number", val: stats.total, icon: Phone, color: "#10B981" },
  ] : [];
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-3)] mt-0.5">
            {total.toLocaleString()} people with phone ·{" "}
      ...
      {statRows.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="card p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-green-400">Phone Only</p>
              <p className="text-[10px] text-[var(--text-3)] uppercase tracking-wide">Contactable via phone</p>
            </div>
          </div>
        </div>
      )}

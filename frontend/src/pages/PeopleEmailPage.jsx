...
import {
  Mail, Search, Filter, Download, RefreshCw,
  Phone, MapPin, ChevronDown, ChevronUp, X,
  Globe, Building2, Database, Users,
} from "lucide-react";
import { fpeGetDatabase, fpeGetStats, fpeGetColumns, fpeRefresh } from "../api";
...
export default function PeopleEmailPage() {
  ...
  const loadColumns = useCallback(async () => {
    try {
      const { data } = await fpeGetColumns();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await fpeGetDatabase({
        page, limit,
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    fpeGetStats().then(({ data }) => setStats(data)).catch(() => {});
  }, []);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      await Promise.all([
        loadColumns(),
        load(),
        fpeGetStats().then(({ data }) => setStats(data)),
      ]);
  ...
  const exportCSV = () => {
    ...
    a.download = "people_email_data.csv";
    a.click();
  };

  const pages    = Math.ceil(total / limit);
  const statRows = stats ? [
    { label: "People with Email", val: stats.total, icon: Mail, color: "#3B82F6" },
  ] : [];
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-3)] mt-0.5">
            {total.toLocaleString()} people with email ·{" "}
      ...
      {statRows.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="card p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-400">Email Only</p>
              <p className="text-[10px] text-[var(--text-3)] uppercase tracking-wide">Contactable via email</p>
            </div>
          </div>
        </div>
      )}

...
import { Database, Search, Filter, Download, RefreshCw, Loader2, Phone, Globe, MapPin, Sparkles, X, ChevronDown, ChevronUp, ExternalLink, Star } from "lucide-react";
...
export default function InBuildDatabasePage() {
  ...
  const exportCSV = () => {
    const hdrs = ["Name","Category","City","Rating","Reviews","Phone","Address","Website"];
    const rows = leads.map(l => [
      l.name||"", l.category||"", l.city_file||"", l.rating||"", l.reviews||"",
      l.phone||"", l.address||"", l.website||""
    ]);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-3)] mt-0.5">
            {total.toLocaleString()} records · business_data
          </p>
        </div>
              ...
              {loading ? (
                Array.from({ length: 6 }).map((_, idx) => (
                  <tr key={`skel-${idx}`} className="border-b border-[var(--border)] animate-pulse">
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-48 mb-1"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-24"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-24"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-16"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-12"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-28"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-40"></div></td>
                    <td className="px-3 py-3"><div className="h-4 bg-[var(--surface-2)] rounded w-20"></div></td>
                  </tr>
                ))
              ) : leads.length === 0 ? (
                <tr><td colSpan={8} className="py-16 text-center text-[var(--text-3)]">
                ...
                <tr key={l.id||i} className="border-b border-[var(--border)] hover:bg-[var(--surface-2)] transition-colors">
                  <td className="px-3 py-2.5">
                    <p className="font-semibold text-[var(--text)]">{l.name||"—"}</p>
                    {l.url && (
                  ...
                  <td className="px-3 py-2.5">
                    {l.category ? <span className="badge badge-purple text-[9px]">{l.category}</span> : "—"}
                  </td>
                  <td className="px-3 py-2.5 text-[var(--text-2)]">{l.city_file?.replace(/_/g," ")||"—"}</td>
                  <td className="px-3 py-2.5">
                    <span className="flex items-center gap-1 text-[var(--text-2)]"><Star size={10} className="text-yellow-500"/>{l.rating||"—"}</span>
                  </td>
                  <td className="px-3 py-2.5 text-[var(--text-3)]">{l.reviews||"—"}</td>
                  <td className="px-3 py-2.5">
                    {l.phone ? <p className="flex items-center gap-1 text-[var(--text-2)]"><Phone size={9}/>{l.phone}</p> : <span className="text-[var(--text-3)]">—</span>}
                  </td>
                  <td className="px-3 py-2.5 text-[var(--text-3)] min-w-[150px]">
                    {l.address && <span className="flex items-start gap-1"><MapPin size={10} className="mt-0.5 flex-shrink-0"/>{l.address}</span>}
                  </td>
                  <td className="px-3 py-2.5">
                    {l.website ? <a href={l.website} target="_blank" rel="noreferrer" className="text-[var(--text-3)] hover:text-blue-400 flex items-center gap-1"><Globe size={10}/>Visit</a> : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
